<?php
include "admin/db.php";  // Adjust if needed based on your folder structure
...
error_log("Slider script started at " . date('Y-m-d H:i:s'));

// Fetch at most 2 slides
$sql = "SELECT * FROM slider ORDER BY id DESC LIMIT 2";  // Fetch latest 2 slides
...
if (!$slider) {
    ...
    $rowCount = mysqli_num_rows($slider);
    error_log("Rows found: " . $rowCount);
    if ($rowCount == 0) {
        echo "No rows found in slider table.";
        $useStatic = true;
    } else {
        $useStatic = false;
    }
        ...
        <?php if (!$useStatic): ?>
            <?php while ($row = mysqli_fetch_assoc($slider)): ?>
                ...
                <div class="owl-carousel-item position-relative">
                    <img class="img-fluid"
                         src="<?= $imagePath ?>"
                         alt="<?= htmlspecialchars($row['heading'] ?? 'Slide Image') ?>"
                        ...
                        <div class="container">
                            <div class="row justify-content-start">
                                <div class="col-10 col-lg-8 text-white">
                                    <h1 class="display-2 animated slideInDown mb-4 text-white" style="color: white;">
                                        <?= !empty($row['heading']) ? htmlspecialchars($row['heading']) : '<em>No Heading Set</em>' ?>
                                    </h1>
                                    <p class="fs-5 fw-medium mb-4 pb-2">
                                        <?= !empty($row['paragraph']) ? htmlspecialchars($row['paragraph']) : '<em>No Paragraph Set</em>' ?>
                                    </p>
                                    <?php if (!empty($row['btn1_text'])): ?>
                                        <a href="<?= htmlspecialchars($row['btn1_url'] ?? '#') ?>" class="btn btn-primary rounded-pill py-sm-3 px-sm-5 me-3 animated slideInLeft"><?= htmlspecialchars($row['btn1_text']) ?></a>
                                    <?php else: ?>
                                        <em>No Button 1 Text</em>
                                    <?php endif; ?>
                                    <?php if (!empty($row['btn2_text'])): ?>
                                        <a href="<?= htmlspecialchars($row['btn2_url'] ?? '#') ?>" class="btn btn-dark rounded-pill py-sm-3 px-sm-5 animated slideInRight"><?= htmlspecialchars($row['btn2_text']) ?></a>
                                    <?php else: ?>
                                        <em>No Button 2 Text</em>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <div class="text-center text-white py-5">No dynamic slides available.</div>
        <?php endif; ?>
    </div>
</div>
    ...
    <div class="container">
        <div class="row g-5 align-items-center">
            <div class="col-lg-6 wow fadeInUp" data-wow-delay="0.1s">
                ...
                <p class="mb-4"><?= $card['description2']; ?></p>
                <div class="row g-4 align-items-center">
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                        <div class="d-flex align-items-center">
                            <img class="rounded-circle flex-shrink-0" 
                                 src="admin/<?= $card['principal_image']; ?>" 
                                 alt="" style="width: 45px; height: 45px; object-fit:cover;">
                            <div class="ms-3">
                                <h6 class="text-primary mb-1"><?= $card['principal_name']; ?></h6>
                                <small><?= $card['principal_designation']; ?></small>
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="container-xxl py-5">
    ...
    <p>Explore our well-structured school classes designed to provide a nurturing learning environment, where students gain knowledge, skills, and confidence for a bright future.</p>
        ...
        <div class="row g-4">
            ...
            // Apne database connection variable ($conn) ka naam check kar lein.
            $classes_query = "SELECT * FROM classes ORDER BY id ASC";
            $classes_result = $conn->query($classes_query);

            // STEP 2: Check karein ki data mila hai ya nahi, taaki error na aaye.
            if ($classes_result && $classes_result->num_rows > 0) {
                // STEP 3: Agar data mila hai, toh ek-ek karke display karein.
                while ($class = $classes_result->fetch_assoc()) {
            ?>
            <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
                <div class="classes-item">
                    <div class="bg-light rounded-circle w-75 mx-auto p-3">
                        <img class="img-fluid rounded-circle" src="admin/<?= htmlspecialchars($class['class_image']) ?>" alt="<?= htmlspecialchars($class['title']) ?>">
                    </div>
                    <div class="bg-light rounded p-4 pt-5 mt-n5">
                        <a class="d-block text-center h3 mt-3 mb-4" href=""><?= htmlspecialchars($class['title']) ?></a>
                        <div class="d-flex align-items-center justify-content-between mb-4">
                            <div class="d-flex align-items-center">
                                <img class="rounded-circle flex-shrink-0" src="admin/<?= htmlspecialchars($class['teacher_image']) ?>" alt="<?= htmlspecialchars($class['teacher_name']) ?>" style="width: 45px; height: 45px;">
                                <div class="ms-3">
                                    <h6 class="text-primary mb-1"><?= htmlspecialchars($class['teacher_name']) ?></h6>
                                    <small>Teacher</small>
                                </div>
                            </div>
                            <span class="bg-primary text-white rounded-pill py-2 px-3"><?= htmlspecialchars($class['price']) ?></span>
                        </div>
                        <div class="row g-1">
                            <div class="col-4">
                                <div class="border-top border-3 border-primary pt-2">
                                    <h6 class="text-primary mb-1">Age:</h6>
                                    <small><?= htmlspecialchars($class['age_range']) ?></small>
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="border-top border-3 border-success pt-2">
                                    <h6 class="text-success mb-1">Time:</h6>
                                    <small><?= htmlspecialchars($class['time']) ?></small>
                                </div>
                            </div>
                            <div class="col-4">
                                <div class="border-top border-3 border-warning pt-2">
                                    <h6 class="text-warning mb-1">Capacity:</h6>
                                    <small><?= htmlspecialchars($class['capacity']) ?></small>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <?php 
                } // Loop yahan khatam hua
            } else {
                // Agar database mein koi class nahi hai, toh yeh message dikhayein
                echo "<p class='text-center'>No classes available at the moment.</p>";
            }
            ?>
        </div>
    </div>
...
<div class="container-xxl py-5">
    <div class="container">
        <div class="text-center mx-auto mb-5 wow fadeInUp" data-wow-delay="0.1s" style="max-width: 600px;">
            ...
            <p>Here’s what our happy clients have to say about us.</p>
        </div>
